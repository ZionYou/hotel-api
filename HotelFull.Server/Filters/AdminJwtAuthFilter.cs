...
public class AdminJwtAuthFilter : IAsyncAuthorizationFilter
{
    private readonly TokenValidationParameters _tokenValidationParameters;
    private readonly ILogger<AdminJwtAuthFilter> _logger;

    public AdminJwtAuthFilter(IOptions<JwtSettings> jwtSettings, ILogger<AdminJwtAuthFilter> logger)
    {
        _logger = logger;
    ...
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context) //實現了授權邏輯，處理傳入請求的身份驗證
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(roleClaim) || roleClaim != "Admin")
            {
                SetUnauthorizedResult(context, string.IsNullOrWhiteSpace(roleClaim) ? "Role claim is missing." : "Only Admin can perform this action.");
                return;
            }
